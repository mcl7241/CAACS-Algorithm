...
class AntGraph:
    def __init__(self, instance_name, num_ants, num_nodes, delta_mat, carbon_mat, clusters_mat, tau_mat=None):
        print (len(delta_mat))
    ...
    def create_image(self, iteration, best_path_vec, best_path_cost):
        ...
        ax.set_ylabel('')
        '''
        c_cost = self.carbon_cost(best_path_vec, self.carbon_mat)
        ax.set_title('Iter:' + iteration + ', ' + 'Cost:' + str(best_path_cost) + ', ' + 'Carbon:' + str(int(c_cost)), fontsize=6)
        # save the plot to a file
        directory_path = './GTSP_image/' + self.instance_name + '/'
