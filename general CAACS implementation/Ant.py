...
class Ant(Thread):
    ...
    def state_transition_rule(self, curr_node):
        ...
        if q < self.Q0:
            ...
            for node in self.nodes_to_visit.values():
                ...
                # CHECK
                val = graph.tau(curr_node, node) * math.pow(graph.etha(curr_node, node), self.Beta) * graph.scaled_emission(curr_node, node)
            ...
            for node in self.nodes_to_visit.values():
                ...
                # CHECK
                probability = tau * math.pow(etha, self.Beta) * graph.scaled_emission(curr_node, node)
            ...
            if sum != 0:
                ...
                max_node = random.choices(nodes)[0]
    ...
    def local_updating_rule(self, curr_node, next_node):
        graph = self.colony.graph
        # CHECK
        val = (1 - self.Rho) * graph.tau(curr_node, next_node) + (self.Rho * graph.tau0 * graph.scaled_emission(curr_node, next_node))
