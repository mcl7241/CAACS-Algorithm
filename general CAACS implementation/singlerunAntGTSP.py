...
def solve(file_path, input_file):
    ...
    for i in range(len(carbon_mat)):
        ...
        for j in range(len(carbon_row)):
            scaled_num = 50**(1 - carbon_mat[i][j]/max_carbon)
    ...
    try:
        ...
        return best_path_vec, best_path_cost, c_cost
...
if __name__ == "__main__":   
    ...
    #output_file.write('Project,Cost,Carbon, Time\n'.encode('utf-8'))

    '''
    files = [f for f in os.listdir(file_path)]
    for input_file in files:
        print ('Processing ', file_path, input_file)
        file_name = '20rd100.txt'
        ...
        #output_file.write((input_file[:-4] + ',' + str(best_path_cost) + ',' + str(c_cost) + ',' + str(time_taken) + '\n').encode('utf-8'))
    '''
